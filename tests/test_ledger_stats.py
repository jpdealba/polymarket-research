...
def test_ledger_stats_output_labels_gross_base_not_fee_adjusted(settings, session, monkeypatch):
    ...
    assert "OrderFilled fee fields are not yet trusted as user-paid actual fees" in result.output
    ...
    assert "fee_scenario_after_2026_03_30      0.402400" in result.output
    assert "estimated_net_pnl_scenario         -54.402400" in result.output
